...
public class VcfWindow {
     ...
     */
    public VcfWindow(SampleFileIterator<VcfEmission> it, boolean check_iter) {
        if (it.hasNext()==false && check_iter) {
    ...
    public static VcfWindow empty() {
        return new VcfWindow(new SampleFileIterator<VcfEmission>() {
            @Override
