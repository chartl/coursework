...
public final class VcfWriter {
    ...
    public static void appendRecords(SampleHapPairs haps,
            GenotypeValues gv, int start, int end, PrintWriter out,
                                     boolean ignorePhase, String[] initialIds) {
        if (start > end) {
        ...
        if (initialIds != null) {
            checkIds(haps, gv, initialIds);
        }
        float[] sumAndAltDose = new float[2];
        sumAndAltDose[0] = 0.0f;
        sumAndAltDose[1] = 0.0f;
        for (int marker=start; marker<end; ++marker) {
            ...
            for (int hp=0, n=haps.nSamples(); hp<n; ++hp) {
                ...
                out.print(ignorePhase ? Const.unphasedSep : Const.phasedSep);
    ...
    public static void appendRecords(SampleHapPairs haps, GenotypeValues gv, int start, int end, PrintWriter out) {
        appendRecords(haps, gv, start, end, out, false);
    }
    ...
    public static void appendRecords(SampleHapPairs haps, GenotypeValues gv, int start, int end, PrintWriter out,
                                                        boolean ignorePhase) {
        appendRecords(haps, gv, start, end, out, ignorePhase, null);

    }

    private static void checkIds(SampleHapPairs haps, GenotypeValues gv, String[] idOrder) {
        for ( int s = 0; s < haps.nSamples(); s ++ ) {
            String hapId = haps.samples().id(s);
            if ( ! idOrder[s].equals(hapId) ) {
                throw new IllegalStateException(String.format("Haplotype samples do not match at %d: %s != %s",
                        s, idOrder[s], hapId));
            }
        }
    }
