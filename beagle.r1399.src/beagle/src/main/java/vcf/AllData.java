...
public class AllData implements Data {
    ...
    public static Data refOnly(File ref, Filter<String> sampleFilter, Filter<Marker> markerFilter,
                               ChromInterval chromInterval, File pedFile, boolean usePhase,
                               float maxLR, boolean impute) {
        assert impute == false;
        System.out.println("Ref only data");
        SampleFileIterator<VcfRecord> filtRefIt = VcfIterator.filteredIterator(
                ref, sampleFilter, markerFilter, chromInterval);
        SampleFileIterator<VcfEmission> refIt = new VcfRefIterator(filtRefIt);
        VcfWindow refWindow = new VcfWindow(refIt);
        System.out.printf("the ref window can advance? %s%n", refWindow.canAdvanceWindow());

        AllData ad = new AllData(refWindow, VcfWindow.empty());
        System.out.printf("AD has %d markers%n", ad.nMarkers());
        System.out.printf("AD emissions=%s%n", ad.refEmissions.toString());
    ...
    /**
     * For AllData that only contain reference data (TODO make a RefAllData subclass)
     *
     * @param requestedOverlap - the requested overlap between the previous window
     * @param windowSize - the number of markers
     */
    public void advanceRefWindow(int requestedOverlap, int windowSize) {
        refData = refWindow.advanceWindow(requestedOverlap, windowSize);
        refEmissions = new RefGL(refWindow.samples(), refData);
        ++window;
        setRefHaplotypes(refEmissions.markers(), refData);  // the whole purpose is here
    }

    public void advanceTargetWindow(int requestedOverlap, int windowSize) {
        targetData = targetWindow.advanceWindow(requestedOverlap, windowSize);
        targetIndices = targetIndices(targetData);
        targetEmissions = new BasicGL(targetWindow.samples(), targetData);
        ++window;

    }

    public boolean canAdvanceRefWindow() { return refWindow.canAdvanceWindow(); }
    public boolean canAdvanceTargetWindow() { return targetWindow.canAdvanceWindow(); }
    ...
    public VcfEmission[] refData() { return refData; }
