...
public final class BitSetGT implements VcfEmission {
    ...
    /**
     * Wrap a refGT into a BitSetGT
     * @param refGT the RefGT
     */
    public BitSetGT(BitSetRefGT refGT) {
        bitsPerAllele = (byte) refGT.bitsPerAllele;
        samples = refGT.samples();
        marker = refGT.marker;
        isRefData = refGT.isRefData();
        allele1 = refGT.allele1;
        allele2 = refGT.allele2;
        isMissing1 = new BitSet(nSamples());
        isMissing2 = new BitSet(nSamples());
        isPhased = new BitSet(nSamples());
    ...
    private BitSetGT(BitSetRefGT refGT, Samples allSamples) {
        bitsPerAllele = (byte) refGT.bitsPerAllele;
        samples = allSamples;
        marker = refGT.marker;
        isRefData = false;
        ...
        isPhased = new BitSet(nSamples());
        int j = 0;
        for (String id : allSamples.ids()) {
            if ( refGT.samples.index(id) == -1 ) {
                // non-founder
                isMissing2.set(j);
                isMissing1.set(j);
            } else {
    ...
    public static BitSetGT extendRefSamples(BitSetRefGT ref, Samples samplesWithRef) {
        return new BitSetGT(ref, samplesWithRef);
    }
