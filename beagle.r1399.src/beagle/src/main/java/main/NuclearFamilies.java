...
public class NuclearFamilies {
    ...
    public String toString() {
        ...
        bldr.append(String.format("NuclearFamilies: %d samples, %d duos, %d trios\n",
                nSamples(), nDuos(), nTrios()));
        for (int sampleIndex = 0; sampleIndex < nSamples(); sampleIndex++) {
            if (mother(sampleIndex) != -1) {
                bldr.append(String.format("sample_id=%s\tmother=%s\tfather=%s\n",
                        samples.idIndex(sampleIndex),
                        samples.idIndex(mother(sampleIndex)),
                        samples.idIndex(father(sampleIndex))));
            } else {
                bldr.append(String.format("sample_id=%s\tFOUNDER\n", samples.idIndex(sampleIndex)));
            }
        }
