...
import jaccard.JaccardEstimator;
import jacquard.Jacquard;
import pedsim.PedigreeSimulator;
import vcf.AllData;
import vcf.Data;
import vcf.GL;
import vcf.Marker;
...
public class Main {
	...
	Locale.setDefault(Locale.US);
        ...
        // Here's where I've uglied up the logic...
        if ( par.simped() ) {
            // there is no phasing, and the non-founders are not expected in the VCF
            main.simulatePedigree();
        } else if ( par.jaccard() || par.jacquard() ) {
            Jacquard.estimateCoefficients((AllData) data, par);
        } else {
            main.phaseData();
        }
    ...
    private void simulatePedigree() {
        System.out.println("Simulation is hooked up.");
        PedigreeSimulator.runSimulation(par, allData(par));
    }
    ...
    private static Data allData(Parameters par) {
        ...
        Filter<Marker> markerFilter = markerFilter(par);
        ChromInterval chromInterval = chromInterval(par);
        if (par.gt() == null && par.gl() == null && par.gtgl() == null && par.ref() != null ) {
            return AllData.refOnly(par.ref(), sampleFilter, markerFilter, chromInterval, par.ped(),
                    par.usephase(), par.maxlr(), par.impute());
        }
        if (par.gt()!=null) {
            assert par.gl()==null && par.gtgl()==null;
            ...
            assert par.gt()==null && par.gl()==null && par.gtgl()!=null;  // this assert is broken (hah!)
    ...
    private static Parameters parameters(String[] args) {
        ...
        if (par.simped() == true && par.ref() == null) {
            String s = "ERROR: The \"simped=true\" parameter requires a phased reference panel";
            Utilities.exit(Parameters.usage() + s);
        }
        if (par.simped() == true && par.impute() == true) {
            String s = "ERROR: The \"impute=false\" parameter is required when a ref panel is specified "
                       + "and \"simped=true\"";
            Utilities.exit(Parameters.usage() + s);
        }
        if (par.simped() == true && par.ped() == null) {
            String s = "ERROR: The \"simped=true\" parameter requires a pedigree to simulate.";
            Utilities.exit(Parameters.usage() + s);
        }
    ...
    private static void checkForOneInputFile(Parameters par) {
        ...
        if (cnt != 1 && ! par.simped() && ! par.jaccard() ) {  // maybe ugly?
